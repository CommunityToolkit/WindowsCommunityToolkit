    ...
    public class HowToGuard
    {
        ...
        public static HowToGuard Symbol(ISymbol target)
        {
            ...
            else if (plat.Kind == PlatformKind.Uwp && target.Kind != SymbolKind.NamedType)
            {
                ...
                if (!d1.TryGetValue(g.TypeToCheck, out List<NewMember> newMembers))
                {
                    d0.TryGetValue(g.TypeToCheck, out newMembers);
                }
                ...
                else if (target.Kind == SymbolKind.Method && plat.ByParameterCount)
                {
