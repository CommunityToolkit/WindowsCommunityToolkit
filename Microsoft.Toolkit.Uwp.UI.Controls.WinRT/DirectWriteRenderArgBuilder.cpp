...
namespace winrt::Microsoft_Toolkit_Uwp_UI_Controls_WinRT::implementation
{
    ...
    DirectWriteRenderArgBuilder::DirectWriteRenderArgBuilder()
    {
        ...
        m_builtArgs.fontFamily = winrt::to_hstring(L"Segoe UI");
        ...
        m_builtArgs.textLocale = winrt::to_hstring(L"en-US");
        m_builtArgs.textWrapping = DWRITE_WORD_WRAPPING::DWRITE_WORD_WRAPPING_NO_WRAP;
        m_builtArgs.fontCollection = nullptr;
    }

    void DirectWriteRenderArgBuilder::SetFontFamily(FontFamily const& fontFamily)
    {
        winrt::hstring resultFontFamily{};

        // try to get the FontFamily property first.
        auto controlFontFamily = fontFamily;
        if ((controlFontFamily != nullptr) && !controlFontFamily.Source().empty())
        {
        ...
        if (resultFontFamily.empty() && (controlFontFamily != nullptr))
        {
            auto xamlDefault = controlFontFamily.XamlAutoFontFamily();
            if ((xamlDefault != nullptr) && !xamlDefault.Source().empty())
            {
        ...
        if (resultFontFamily.empty())
        {
            resultFontFamily = winrt::to_hstring(L"Segoe UI");
        }
    ...
    void DirectWriteRenderArgBuilder::SetText(winrt::hstring const& text)
    {
        m_builtArgs.text = text;
    }

    void DirectWriteRenderArgBuilder::SetTextLocale(winrt::hstring const& textLocale)
    {
    ...
    void DirectWriteRenderArgBuilder::SetForegroundBrush(Brush const& brush)
    {
        auto solidColorBrush{ brush.try_as<SolidColorBrush>() };
    ...
    void DirectWriteRenderArgBuilder::BuildFontCollection(const winrt::hstring& fontFamily)
    {
        ...
        if (UniversalWindowsAppPackageFontLoader::FontCollectionLoader::HasCustomFontFamily(fontFamily))
        {
            ...
            auto fontFamilyString = fontFamily.c_str();
            auto fontFamilySize = fontFamily.size() * sizeof(wchar_t);
            ...
            m_builtArgs.fontFamily = parseData.customFontName;
        }
        else
        {
