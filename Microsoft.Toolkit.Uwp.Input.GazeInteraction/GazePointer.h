...
public:
    ...
    winrt::event<GazeEventArgs> GazeEvent;

	/// <summary>
	/// The UIElement representing the cursor.
	/// </summary>
	UIElement CursorElement() { return _gazeCursor.PopupChild; }
	void CursorElement(UIElement const& value) { _gazeCursor.PopupChild = value; }

private:

    EventHandler<GazeEventArgs> _gazeEvent;
    GazeEventArgs const _gazeEventArgs = GazeEventArgs();
    int _gazeEventCount = 0;

    Brush _enterBrush = nullptr;

    Brush _progressBrush = SolidColorBrush(Colors::Green());

    Brush _completeBrush = SolidColorBrush(Colors::Red());

    double _dwellStrokeThickness = 2;

    Interaction _interaction = Interaction::Disabled;

    GazeTargetItem _nonInvokeGazeTargetItem = nullptr;
    ...
    bool IsCursorVisible() { return _gazeCursor.IsCursorVisible; }
    void IsCursorVisible(bool const& value) { _gazeCursor.IsCursorVisible = value; }
