...
namespace UnitTests.WinUI.UI.Controls
{
    ...
    public class Test_RichSuggestBox : VisualUITestBase
    {
        ...
        public async Task Test_RichSuggestBox_CustomizeToken()
        {
            await App.DispatcherQueue.EnqueueAsync(async () =>
            {
                ...
                rsb.SuggestionChosen += (rsb, e) =>
                {
                    ...
                    var format = e.Format;
                    format.BackgroundColor = Microsoft.UI.Colors.Beige;
                    format.ForegroundColor = Microsoft.UI.Colors.Azure;
                ...
                Assert.AreEqual(Microsoft.UI.Colors.Beige, range.CharacterFormat.BackgroundColor, "Unexpected token background color.");
                Assert.AreEqual(Microsoft.UI.Colors.Azure, range.CharacterFormat.ForegroundColor, "Unexpected token foreground color.");
        ...
        public async Task Test_RichSuggestBox_FormatReset()
        {
            ...
            {
                var rsb = new RichSuggestBox() { Prefixes = "@" };
                rsb.TokenBackground = new Microsoft.UI.Xaml.Media.SolidColorBrush(Microsoft.UI.Colors.Azure);
                await SetTestContentAsync(rsb);
                var document = rsb.TextDocument;
                var selection = document.Selection;
